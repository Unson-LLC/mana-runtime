...
raise "deploy must bind the installed control plane to the workflow commit" unless ssh_step&.dig("env", "CONTROL_PLANE_DIGEST") == "${{ steps.control_plane.outputs.digest }}"
...
raise "summary must receive the control-plane commit output" unless summary_step&.dig("env", "CONTROL_PLANE_SHA") == "${{ steps.control_plane.outputs.sha }}"
