...
- [ ] AC19: 本番配備後、実Slack E2Eの前に、認証済み管理APIから本番と同じClaudeコマンド・設定・JSON Schema・Judgment Hook・Receipt注入を使う議事録生成プローブを実行できる。成功時は生成内容を返さず、失敗時は許可済みの診断コードだけを返す。プローブ成功を確認するまで利用者のSlack投稿を再実行させない。
