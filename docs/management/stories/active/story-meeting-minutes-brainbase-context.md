...
- [ ] AC18: 「取り消して選び直す」の確認後は、同じSlack投稿を即座に「保存先をやり直しています」へ切り替える。GitHub・Canonical Task・共有Slackの取り消しをrunへ工程別に永続化し、途中失敗後の再実行は完了済み工程を重複させない。成功時は同じ投稿を保存先選択へ、失敗時は理由付きの再実行表示へ必ず切り替え、確認画面を残し続けない。
