...
今回の境界ハンドルsliceでは、受理済みouter envelopeをQueue decision snapshotと既存opaque handleの任意のdurable recordへ保持し、trusted local boundary handlerが`container_launch`、`mcp_gateway`、`brainbase_proxy`、`slack_delivery`でouter／nested／payloadを再検証する。`approval`／`human_action`は`require_auto` guardでstorage／effect前に拒否する。これは共通primitiveのローカル実装・検証であり、実Company Authority operationのproduction provider接続、live runtime、同一run本番証跡は未接続・`not_collected`である。
