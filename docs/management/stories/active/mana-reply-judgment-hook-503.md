...
- [ ] AC8: A0の外部効果照合が同一Slack messageのruntime claimを保持している間、T0の同一配送はQueueをackして照合へ委譲する。通常の`in_progress` claimは従来どおりQueue再試行を維持する。
