...
A0が外部効果のreadbackを待つために`preserveUntilReconciled`を付けて保持したruntime claimは、通常のlease中claimと区別してT0へ返す。T0は保持claimを所有せず、同じcanonical Slack messageについてQueue再試行を発生させず成功としてackする。A0は引き続き唯一の外部効果照合・送信責任者である。保持されていない`in_progress`は、既存どおり`UPSTREAM_UNAVAILABLE`で再試行し、所有者の完了またはlease回収を待つ。claim tokenや外部効果の内容をT0へ渡さない。
...
- runtime claim regression: 保持claimの`in_progress`識別子、通常claimの識別子、保持claimだけをT0がackする分岐を固定する
