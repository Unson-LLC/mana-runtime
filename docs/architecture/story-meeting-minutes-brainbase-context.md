...
Slackのスレッド特定、即時状態、選択確認、組織・戻る操作、受付停止、保存先のやり直し、ライフサイクル、Task操作で利用者へ表示する失敗には、`runId`・失敗段階・固定エラーコードから導出した決定的な`問い合わせID`を必ず付ける。実行前のrouter受付停止通知は受信した`eventId`をrun相当の相関シードにし、保存先選択・やり直しのQueue通知はcommandの`runId`を使う。旧形式などで識別子を受け取れない通知は、channel・threadまたはchannel・userから構成する決定的なfallback seedを使い、空のシードやraw errorを問い合わせIDにしない。受付停止判定と`mana_task_write_approve`のコールバック拒否は、raw errorを返さずHTTP 503の安全な失敗envelopeへ変換し、資格条件を満たす署名済み`response_url`だけへ安全投影する。遅延Taskのopen・edit・cancelはPromiseをraw rejectで残さず、固定診断ログと安全投影へ収束させる。scope不一致の既存通知は維持する。
