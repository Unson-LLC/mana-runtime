...
export function classifyMeetingMinutesFailure(stage: MeetingMinutesDiagnosticStage, error: unknown):
  ...
  const exact: Record<string, [string, boolean]> = {
    ...
    meeting_minutes_context_identity_missing: ["CONTEXT_IDENTITY_MISSING", false],
    meeting_minutes_context_receipt_missing: ["CONTEXT_RECEIPT_MISSING", false],
    meeting_minutes_context_changed: ["CONTEXT_CHANGED", false],
