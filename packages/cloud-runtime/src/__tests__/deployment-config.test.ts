...
describe("会社別Cloudflare deployment", () => {
  ...
  it("wires signed interactions and meeting-minutes Queue handling without removing task entrypoints", () => {
    ...
    expect(worker).toContain('env.BRAINBASE_TASK_API_BASE_URL ?? "", env.BRAINBASE_TASK_API_TOKEN, credentialFetch)');
    expect(worker).not.toContain('env.BRAINBASE_TASK_API_BASE_URL ?? "", env.BRAINBASE_TASK_API_TOKEN).resolve(identity, receiptId)');
