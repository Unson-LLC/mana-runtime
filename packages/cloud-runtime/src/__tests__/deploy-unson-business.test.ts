...
describe("unson business deploy wrapper", () => {
  ...
  it("stops before Wrangler when the production workspace connection is unavailable", async () => {
    const script = fileURLToPath(new URL("../../scripts/deploy-unson-business.mjs", import.meta.url));
    ...
    try {
      ...
      expect(result.stderr).toContain(
        "tenant_runtime_deploy_preflight_failed:BRAINBASE_WORKSPACE_CONNECTIONS_JSON",
      );
