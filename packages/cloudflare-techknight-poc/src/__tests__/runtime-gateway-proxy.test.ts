...
const placements = JSON.stringify([{ placementId: "mana-dev-biz", channelId: "C_MANA", projectCodes: ["mana"], taskWriteEnabled: true,
  audience: { type: "operator", allowedUserIds: ["UALLOWED"] }, capabilities: { mcp: ["gateway"], gatewayTools: ["list_tasks","search_tasks","send_message"] },
...
describe("runtime gateway proxy", () => {
  ...
  it("lists employees without persona and returns an exact employee", async () => {
    const listPlacement = placements.replace('"search_tasks","send_message"', '"search_tasks","send_message","list_employees","get_employee"');
  ...
  it("reads sessions only from the placement registry object", async () => {
    const sessionPlacement = placements.replace('"search_tasks","send_message"', '"search_tasks","send_message","list_sessions","get_session"');
