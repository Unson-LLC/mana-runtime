...
const tenantContext = async (repair: { channelId: string; requestedAt: string; targetId: string }) => ({
  ...
  slack: {
    event_id: `task-board-repair:${repair.targetId}:${repair.requestedAt}`,
    channel_id: repair.channelId,
    thread_ts: repair.requestedAt,
    requester_id: "svc_canvas",
  },
} as never);
...
describe("Worker task Canvas repair producers", () => {
  ...
  it("suppresses disabled project fanout and enqueues an exact binding snapshot", async () => {
    ...
    const info = vi.spyOn(console, "info").mockImplementation(() => undefined);
    const activeTarget = { ...disabledTarget, targetId: "minutes-pms-owned", channelId: "C0BKX9Y1700", enabled: true,
