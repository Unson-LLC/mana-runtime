...
describe("Slack native development command", () => {
  it("acks an authorized command and queues a deterministic development event", async () => {
    ...
    const response = await handleSlackCommandRequest(make(body), { signingSecret: secret, expectedTeamId: "T1", placements: [{ channelId: "C1", allowedUserIds: ["U1"] }], nowMs, send });
  ...
  it("does not combine a user from one placement with another placement channel", async () => {
    ...
    const response = await handleSlackCommandRequest(make(body), { signingSecret: secret, expectedTeamId: "T1",
