...
function buildPrompt(
  ...
  return [
    ...
    ...(taskSearchEnabled ? [
      ...
      "利用者が明示的に複数または他のチャンネルを対象にした場合だけlist_tasks_across_channelsまたはsearch_tasks_across_channelsを使ってください。利用者が示したチャンネル名をchannel_namesへ渡してください。対象名がない全件横断依頼ではlist_authorized_task_channelsを使い、返された全channel_idを横断toolのchannel_idsへ渡してください。利用者へチャンネル名を質問しないでください。channel IDを利用者へ要求しないでください。通常の依頼では現在チャンネル用toolを使ってください。",
