...
const TOOL_DEFINITIONS = Object.freeze([
  ...
  { name: "list_tasks_across_channels", description: "明示的に許可された複数チャンネルのタスクを最大20件一覧します。対象projectはサーバー側で解決します。", inputSchema: { type: "object", additionalProperties: false, required: ["channel_ids"], properties: { channel_ids: { type: "array", minItems: 1, maxItems: 10, uniqueItems: true, items: { type: "string", pattern: "^[A-Z0-9_]{2,32}$" } }, status: { type: "string" }, priority: { type: "string" }, assignee_person_id: { type: "string" }, cursor: { type: "string" }, limit: { type: "integer", minimum: 1, maximum: 20 } } } },
  { name: "search_tasks_across_channels", description: "明示的に許可された複数チャンネルのタスクを最大20件検索します。対象projectはサーバー側で解決します。", inputSchema: { type: "object", additionalProperties: false, required: ["channel_ids","query"], properties: { channel_ids: { type: "array", minItems: 1, maxItems: 10, uniqueItems: true, items: { type: "string", pattern: "^[A-Z0-9_]{2,32}$" } }, query: { type: "string", minLength: 1, maxLength: 200 }, status: { type: "string" }, priority: { type: "string" }, assignee_person_id: { type: "string" }, cursor: { type: "string" }, limit: { type: "integer", minimum: 1, maximum: 20 } } } },
