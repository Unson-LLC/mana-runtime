...
export const taskWriteTools = [
  ...
  { name: "update_task", description: "Brainbaseタスクを楽観ロック付きで更新します。", inputSchema: { type: "object", additionalProperties: false, required: ["project","call_index","task_id","expected_version"], properties: { ...common, task_id: { type: "string" }, expected_version: { type: "integer", minimum: 0 }, title: { type: "string" }, description: { type: "string" }, priority: { enum: ["low","medium","high","urgent"] }, assignee_person_id: { type: ["string","null"] }, due_at: { type: "string" } } } },
  { name: "transition_task", description: "Brainbaseタスクの状態を楽観ロック付きで遷移します。", inputSchema: { type: "object", additionalProperties: false, required: ["project","call_index","task_id","expected_version","to_status"], properties: { ...common, task_id: { type: "string" }, expected_version: { type: "integer", minimum: 0 }, to_status: { enum: ["pending","in_progress","waiting","completed"] }, waiting_on: { type: "string" }, review_at: { type: "string" } } } },
];
